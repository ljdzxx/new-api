...
func newDisconnectingImageStream(t *testing.T, sseBody, disconnectAfter string) (*gin.Context, *httptest.ResponseRecorder, *http.Response, *relaycommon.RelayInfo) {
	...
	resp.Body = &blockingBody{chunk: []byte(sseBody), closed: make(chan struct{})}
	return c, recorder, resp, info
}
...
func TestOpenaiImageHandlerUsesPositiveActualCountForFixedPrice(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "fixed price uses data length", body: `{"data":[{"b64_json":"` + longImage + `"},{"b64_json":"second"}]}`, usePrice: true, wantCount: 2},
		{name: "empty data keeps requested count", body: `{"data":[]}`, usePrice: true, wantCount: 3},
		{name: "ratio billing ignores data length", body: `{"data":[{"b64_json":"first"},{"b64_json":"second"}]}`, usePrice: false, wantCount: 3},
	}
