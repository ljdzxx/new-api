...
func OpenaiImageStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	// A completed/EOF stream provides a trustworthy actual image count. On a
	// client abort, keep the requested count so disconnecting early cannot lower
	// the charge; an already-observed higher count may still raise it.
...
func openaiImageJSONAsStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	// Decode only usage/error. Image base64 values can be several megabytes, so
	// avoid copying them into Go strings before rebuilding the SSE payloads.
	var usageResp dto.SimpleResponse
	...
	for i := int64(0); i < imageCount; i++ {
		...
		// Insert b64_json last because each sjson mutation reallocates the payload.
		...
		if writeErr := writeOpenaiImageStreamChunk(c, payload); writeErr != nil {
