...
// BillingUsage preserves the upstream protocol's native usage semantics while
// the response is converted into another protocol for the client.
...
func NewClaudeMessagesBillingUsage(usage *ClaudeUsage) *BillingUsage {
	...
	return &BillingUsage{Source: BillingUsageSourceClaudeMessages, Semantic: BillingUsageSemanticAnthropic, ClaudeUsage: cloneClaudeUsage(usage)}
}
...
func HasClaudeUsageTokens(usage *ClaudeUsage) bool {
	...
	if usage.InputTokens != 0 || usage.OutputTokens != 0 || usage.CacheCreationInputTokens != 0 ||
		usage.CacheReadInputTokens != 0 || usage.ClaudeCacheCreation5mTokens != 0 || usage.ClaudeCacheCreation1hTokens != 0 {
		return true
	}
...
func newOpenAIBillingUsage(source string, usage *Usage) *BillingUsage {
	...
	return &BillingUsage{Source: source, Semantic: BillingUsageSemanticOpenAI, OpenAIUsage: cloneOpenAIUsage(usage)}
}
...
func HasOpenAIUsageTokens(usage *Usage) bool {
	...
	if usage.PromptTokens != 0 || usage.CompletionTokens != 0 || usage.TotalTokens != 0 || usage.InputTokens != 0 ||
		usage.OutputTokens != 0 || usage.PromptCacheHitTokens != 0 || usage.ClaudeCacheCreation5mTokens != 0 ||
		usage.ClaudeCacheCreation1hTokens != 0 {
		return true
	}
	if usage.PromptTokensDetails.CachedTokens != 0 || usage.PromptTokensDetails.CacheWriteTokens != 0 ||
...
func NewEstimatedGeminiChatBillingUsage(usage *Usage) *BillingUsage {
	...
	return newGeminiChatBillingUsage(&GeminiUsageMetadata{
		PromptTokenCount: usage.PromptTokens, CandidatesTokenCount: usage.CompletionTokens, TotalTokenCount: totalTokens,
	}, true)
}
...
func newGeminiChatBillingUsage(metadata *GeminiUsageMetadata, estimated bool) *BillingUsage {
	...
	clone := cloneGeminiUsageMetadata(*metadata)
	return &BillingUsage{Source: BillingUsageSourceGeminiChat, Semantic: BillingUsageSemanticGemini, Estimated: estimated, GeminiUsageMetadata: &clone}
}
...
func HasGeminiUsageMetadataTokens(metadata *GeminiUsageMetadata) bool {
	...
	if metadata.PromptTokenCount != 0 || metadata.ToolUsePromptTokenCount != 0 || metadata.CandidatesTokenCount != 0 ||
		metadata.TotalTokenCount != 0 || metadata.ThoughtsTokenCount != 0 || metadata.CachedContentTokenCount != 0 {
		return true
	}
